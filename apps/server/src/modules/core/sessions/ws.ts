...
import { SessionsError, SessionsService, sessionIdForEvent } from './service.js';

const MAX_FRAME_BYTES = 256 * 1024;
const MAX_BUFFERED_BYTES = MAX_FRAME_BYTES * 128;
const TRANSIENT_SENSITIVE_KEY = /(?:secret|token|password|authorization|credential|cookie|api[-_]?key|private[-_]?key)/iu;
const TRANSIENT_RUNTIME_ID_KEY = /^(?:runtimeSessionId|nativeSessionId|native_session_id|runtime_session_id)$/u;
const SubscriptionMessageSchema = z.discriminatedUnion('type', [
  z.object({ type: z.literal('subscribe'), sessionId: z.string().min(1).max(160), afterSequence: z.number().int().nonnegative().max(Number.MAX_SAFE_INTEGER).default(0) }),
  z.object({ type: z.literal('unsubscribe'), sessionId: z.string().min(1).max(160) }),
]);
...
export class SessionSubscriptionHub {
  ...
  private isSessionEvent(event: EventEnvelope, sessionId: string): boolean {
    return sessionIdForEvent(this.#database, event) === sessionId;
  }


  private send(subscription: Subscription, payload: unknown): boolean {
    if (subscription.closed || subscription.socket.readyState !== 1 || !this.#service.canRead(subscription.user, subscription.sessionId)) {
      subscription.unsubscribe();
      return false;
    }
...
function sanitizeTransientPayload(payload: SessionTransientPayload, depth = 0): SessionTransientPayload {
  ...
  for (const [key, value] of Object.entries(payload).slice(0, 128)) {
    if (TRANSIENT_SENSITIVE_KEY.test(key) || TRANSIENT_RUNTIME_ID_KEY.test(key)) {
      ...
      output[key] = sanitizeTransientValue(value, depth + 1);
    }
  }
  return output;
}

function sanitizeTransientValue(value: unknown, depth: number): unknown {
  if (depth > 4) return '[REDACTED]';
  if (Array.isArray(value)) return value.slice(0, 128).map((entry) => sanitizeTransientValue(entry, depth + 1));
  if (value && typeof value === 'object') return sanitizeTransientPayload(value as SessionTransientPayload, depth);
  return typeof value === 'string' ? redactText(value.slice(0, 200_000), 200_000) : value;
}
  ...
  return {
    ...
    handle(socket, user) {
      ...
      socket.on?.('message', (raw: unknown) => {
        ...
        const parsed = SubscriptionMessageSchema.safeParse(message);
        if (!parsed.success) { socket.close?.(1008, 'invalid subscription'); return; }
        ...
        subscriptions.get(input.sessionId)?.();
        try {
          subscriptions.set(input.sessionId, hub.subscribe(socket, user, input.sessionId, input.afterSequence));
