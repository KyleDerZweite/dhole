...
function assertTokenTtl(ttlMs: number): number {
  if (!Number.isFinite(ttlMs) || ttlMs <= 0 || ttlMs > 7 * 24 * 60 * 60_000) {
    throw new FleetError('invalid_ttl', 'Enrollment token TTL must be positive and no more than seven days', 422);
  }
...
/**
 * Fleet domain service. It owns enrollment, machine state, allowlists and the
...
export class FleetService {
  ...
  private transaction<T>(operation: () => T): T {
    // Fleet may be called from a larger EventStore/database transaction.  Do
  ...
  private appendAudit(
    ...
    // Fleet team/machine configuration has no required project aggregate. Keep
  ...
  issueEnrollmentToken(
    ...
    const input: IssueEnrollmentTokenInput = typeof inputOrTeam === 'string'
      ...
      : inputOrTeam;
    if (!input.teamId || !input.label || !input.createdBy) throw new FleetError('invalid_enrollment_request', 'teamId, label and createdBy are required', 422);
  ...
  consumeEnrollmentToken(token: string, machineName?: string): EnrollmentResult {
    if (!token || token.length > 512) throw new FleetError('invalid_enrollment_token', 'Enrollment token is invalid', 401);
    ...
    const result = this.transaction(() => {
      ...
      if (!row || row.consumed_at || row.revoked_at || Date.parse(row.expires_at) <= Date.parse(consumedAt)) {
        throw new FleetError('invalid_enrollment_token', 'Enrollment token is invalid, expired, revoked, or already consumed', 401);
      }
      ...
      if (updated.changes !== 1) throw new FleetError('invalid_enrollment_token', 'Enrollment token was already consumed', 409);
      this.database.prepare(`
  ...
  replaceDeviceCredential(machineId: string, actorId?: string): { credentialId: string; credential: string } {
    ...
    const row = this.database.prepare('SELECT id FROM machines WHERE id = ?').get(machineId);
    if (!row) throw new FleetError('machine_not_found', 'Machine not found', 404);
    const credentialId = this.ids.id();
    const credential = this.ids.token(32);
    this.transaction(() => {
  ...
  heartbeat(machineId: string, availableSlots: number, runtimes: readonly unknown[] = []): void {
    ...
    if (!machine) throw new FleetError('machine_not_found', 'Machine not found', 404);
    if (machine.status === 'revoked') throw new FleetError('machine_revoked', 'Machine has been revoked', 403);
    this.transaction(() => {
  ...
  disconnect(machineId: string, socket?: WebSocket): void {
    ...
    this.transaction(() => {
      ...
      this.database.prepare(`UPDATE machines SET status = CASE WHEN status = 'revoked' THEN status ELSE 'disconnected' END, updated_at = ? WHERE id = ?`).run(at, machineId);
  ...
  addRepositoryAllowlist(machineId: string, repositoryId: string, canonicalRoot: string, actorId?: string): void {
    if (!isAbsolute(canonicalRoot)) throw new FleetError('invalid_repository_root', 'Repository root must be absolute', 422);
  ...
  enqueueCommand(inputOrMachine: EnqueueCommandInput | string, commandArg?: NodeCommand, projectIdArg?: string): Record<string, unknown> {
    ...
    if (!parsed.success) throw new FleetError('invalid_command', parsed.error.issues[0]?.message ?? 'Invalid node command', 422);
    ...
    if (input.projectId) {
      ...
      if (!project) throw new FleetError('project_not_allowed', 'Project is not owned by the machine team', 403);
    }
    if ('repositoryId' in command) {
      if (!this.isRepositoryAllowed(input.machineId, command.repositoryId)) {
        throw new FleetError('repository_not_allowed', 'Repository is not allowlisted for this machine', 403);
      }
      if (input.projectId) {
        ...
        if (!repository) throw new FleetError('repository_not_allowed', 'Repository is not part of the command project', 403);
      }
    }
    ...
    if (existing) {
      ...
      if ((existing.project_id ?? null) !== (input.projectId ?? null) || canonicalJson(existingCommand) !== canonicalJson(command)) {
        throw new FleetError('operation_key_conflict', 'Operation key is already bound to a different command', 409);
      }
    ...
    // An outer orchestration transaction owns post-commit delivery; its
    // queueNodeCommand hook schedules a retry.  Direct calls deliver promptly
    // once this method's own transaction has committed.
  ...
  attachConnection(machineId: string, socket: WebSocket, journalOperations: readonly { operationKey: string; state: string }[] = []): void {
    ...
    this.transaction(() => {
      ...
      for (const item of journalOperations) {
        if (!['accepted', 'running', 'uncertain'].includes(item.state)) continue;
        ...
        if ((rank[item.state] ?? 0) < (rank[current.state] ?? 0)) continue;
        const changed = this.database.prepare('UPDATE node_commands SET state = ?, acknowledged_at = COALESCE(acknowledged_at, ?), updated_at = ? WHERE id = ? AND state NOT IN (\'completed\', \'failed\', \'cancelled\', \'expired\')').run(item.state, at, at, command.id);
        if (changed.changes === 1 && (rank[current.state] ?? 0) < 2) {
          this.appendCommandEvent(current.project_id, 'command.acknowledged', command.id, machineId, { operationKey: item.operationKey, state: item.state });
        }
      }
    });
  ...
  handleRuntimeEvent(machineId: string, event: NodeRuntimeEvent): void {
    ...
    const row = this.database.prepare('SELECT project_id FROM node_commands WHERE machine_id = ? AND id = ? AND operation_key = ?').get(machineId, event.commandId, event.operationKey) as { project_id: string | null } | undefined;
  ...
  deliverPending(machineId: string, journal = new Map<string, string>()): void {
    ...
    const rows = this.database.prepare(`SELECT * FROM node_commands WHERE machine_id = ? AND state IN ('queued', 'delivered', 'accepted', 'running') ORDER BY created_at LIMIT 100`).all(machineId) as Array<Record<string, unknown>>;
    for (const row of rows) {
      const linked = this.database.prepare(`
        SELECT e.state AS execution_state, r.state AS run_state, s.state AS session_state
        FROM orchestration_work_items w
        JOIN orchestration_executions e ON e.id = w.execution_id
        JOIN runs r ON r.id = e.run_id
        JOIN sessions s ON s.id = r.session_id
        WHERE w.machine_id = ? AND EXISTS (
          SELECT 1 FROM node_commands c WHERE c.id = ? AND c.machine_id = w.machine_id AND c.operation_key = ?
        )
        LIMIT 1
      `).get(machineId, row.id, row.operation_key) as { execution_state: string; run_state: string; session_state: string } | undefined;
      const linkedTerminal = Boolean(linked && (
        ['settled', 'failed', 'cancelled'].includes(linked.execution_state)
        || ['settled', 'failed', 'cancelled'].includes(linked.run_state)
        || ['failed', 'cancelled', 'closed'].includes(linked.session_state)
      ));
      if (linkedTerminal) {
        ...
        if (row.state === 'queued') {
          ...
          this.transaction(() => {
            ...
            if (changed.changes === 1) {
              ...
              this.appendCommandEvent(projectId, 'command.completed', String(row.id), machineId, { operationKey: String(row.operation_key), state: 'cancelled', reason: 'linked orchestration is terminal' });
            }
          });
        }
        continue;
...
export function authenticateNodeCredential(fleet: FleetService, machineId: string, authorization: string | undefined): boolean {
  const credential = bearerCredential(authorization);
  return Boolean(credential && fleet.authenticateNode(machineId, credential));
}

/** Attach an authenticated outbound node WebSocket to the fleet service. */
export function handleNodeConnection(socket: WebSocket, fleet: FleetService, options: NodeConnectionHandlerOptions = {}): () => void {
  ...
  const onMessage = (data: WebSocket.RawData): void => {
    try {
      ...
      if (!authenticated) {
        ...
        if (!parsed.success || parsed.data.type !== 'hello' || !credential || (options.machineId && options.machineId !== candidateMachineId) || !fleet.authenticateNode(candidateMachineId, credential)) { closeSocketSafely(socket, 4003, 'authentication failed'); return; }
        authenticated = true;
        machineId = candidateMachineId;
        if (timer) clearTimeout(timer);
        timer = undefined;
        fleet.attachConnection(machineId, socket, parsed.data.journalOperations);
        return;
      }
      ...
      const current = fleet.connection(machineId);
      if (!current || current.socket !== socket) { closeSocketSafely(socket, 4003, 'node connection is no longer active'); return; }
      if (parsed.data.type === 'heartbeat') fleet.heartbeat(machineId, parsed.data.availableSlots, parsed.data.runtimes);
      else if (parsed.data.type === 'command_status') fleet.handleStatus(machineId, parsed.data);
      else if (parsed.data.type === 'runtime_event') fleet.handleRuntimeEvent(machineId, parsed.data);
    } catch (error) {
      // FleetError (including a late heartbeat/status from a revoked machine)
      // is a protocol-level rejection, not an uncaught WebSocket exception.
      if (error instanceof FleetError) closeSocketSafely(socket, 4003, error.message);
  ...
  const onClose = (): void => {
    if (timer) clearTimeout(timer);
    if (machineId) {
      try { fleet.disconnect(machineId, socket); } catch { /* socket teardown must remain best-effort */ }
    }
  };
...
export function createNodeConnectionHandler(fleet: FleetService, options: NodeConnectionHandlerOptions = {}): (socket: WebSocket) => () => void {
  return (socket) => handleNodeConnection(socket, fleet, options);
}

function userFromContext(context: Context): { id: string; teamId: string; role: 'administrator' | 'member' } | undefined {
  const user = context.get('user' as never) as { id?: string; teamId?: string; role?: 'administrator' | 'member' } | undefined;
  return user?.id && user.teamId && user.role ? { id: user.id, teamId: user.teamId, role: user.role } : undefined;
}

function requireFleetAdmin(context: Context): { id: string; teamId: string; role: 'administrator' } {
  const user = userFromContext(context);
  if (!user) throw new FleetError('authentication_required', 'Authentication is required', 401);
  if (user.role !== 'administrator') throw new FleetError('administrator_required', 'Administrator access is required', 403);
  return { id: user.id, teamId: user.teamId, role: 'administrator' };
}

function requireFleetOperatorAdmin(context: Context): { id: string; teamId: string; role: 'administrator' } {
  const user = requireFleetAdmin(context);
  ...
  if (!credential || typeof credential.tokenId !== 'string' || credential.tokenId.length < 1 || credential.runId !== undefined || !credential.permissions?.includes('fleet:admin')) {
    throw new FleetError('api_token_required', 'Credential rotation requires an administrator API token', 403);
  }
  return user;
}
...
export function createFleetService(context: ServerContext): FleetService {
  const existing = fleetServices.get(context);
  if (existing) return existing;
  const service = new FleetService(context.database, context.clock, context.ids, context.config.demo ? { staleAfterMs: 24 * 60 * 60 * 1_000 } : {}, context.events);
  fleetServices.set(context, service);
  return service;
}

const fleetServices = new WeakMap<ServerContext, FleetService>();

export const fleetModule: DholeModule = {
  id: 'fleet',
  register(app: DholeApp, context: ServerContext): void {
    const fleet = createFleetService(context);
    const adminMachine = (c: Context): { id: string; teamId: string; role: 'administrator' } => {
      const user = requireFleetAdmin(c);
      const machineId = c.req.param('machineId');
      if (!machineId) throw new FleetError('machine_not_found', 'Machine not found', 404);
      const machine = fleet.getMachine(machineId);
      if (!machine || machine.team_id !== user.teamId) throw new FleetError('machine_not_found', 'Machine not found', 404);
      return user;
    };
    app.post('/api/fleet/enrollment-tokens', async (c) => {
      const body = await parseJson(c, IssueTokenSchema);
      const user = requireFleetAdmin(c);
      if (body.teamId && body.teamId !== user.teamId) throw new FleetError('team_forbidden', 'Enrollment tokens are scoped to your team', 403);
      const issued = fleet.issueEnrollmentToken({ teamId: user.teamId, label: body.label, createdBy: user.id, ...(body.ttlMs === undefined ? {} : { ttlMs: body.ttlMs }) });
      return c.json(issued, 201);
    });
    app.post('/api/fleet/enrollment/consume', async (c) => {
      const body = await parseJson(c, ConsumeTokenSchema);
      return c.json(fleet.consumeEnrollmentToken(body.token, body.machineName));
    });
    app.get('/api/fleet/machines', (c) => {
      const user = userFromContext(c);
      if (!user) throw new FleetError('authentication_required', 'Authentication is required', 401);
      return c.json(fleet.listMachines(user.teamId));
    });
    app.post('/api/fleet/machines/:machineId/allowlist', async (c) => {
      const user = adminMachine(c);
      const body = await parseJson(c, AllowlistSchema);
      const repository = context.database.prepare('SELECT r.id FROM repositories r JOIN projects p ON p.id = r.project_id WHERE r.id = ? AND p.team_id = ?').get(body.repositoryId, user.teamId);
      if (!repository) throw new FleetError('repository_not_found', 'Repository not found', 404);
      fleet.addRepositoryAllowlist(c.req.param('machineId'), body.repositoryId, body.canonicalRoot, user.id);
      return c.json({ ok: true });
    });
    app.get('/api/fleet/machines/:machineId/commands', (c) => { adminMachine(c); return c.json(fleet.listCommands(c.req.param('machineId'))); });
    app.post('/api/fleet/machines/:machineId/commands', async (c) => {
      adminMachine(c);
      const body = await parseJson(c, CommandSchema);
      return c.json(fleet.enqueueCommand({ machineId: c.req.param('machineId'), command: body.command, ...(body.projectId ? { projectId: body.projectId } : {}) }), 202);
    });
    app.post('/api/fleet/machines/:machineId/credential/replace', (c) => {
      const user = requireFleetOperatorAdmin(c);
      const machineId = c.req.param('machineId');
      const machine = fleet.getMachine(machineId);
      if (!machine || machine.team_id !== user.teamId) throw new FleetError('machine_not_found', 'Machine not found', 404);
      return c.json(fleet.replaceDeviceCredential(machineId, user.id));
    });
  },
};
