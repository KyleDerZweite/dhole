...
import { SessionsError, SessionsService, type SessionsFleetApi } from './service.js';
...
function fixture() {
  ...
  database.exec(`INSERT INTO teams(id,name,created_at) VALUES ('team-1','Team','2026-08-30T00:00:00.000Z');
    ...
    INSERT INTO projects(id,team_id,name,created_by,created_at,updated_at) VALUES ('p1','team-1','Project','u1','2026-08-30T00:00:00.000Z','2026-08-30T00:00:00.000Z');`);
...
describe('sessions state machine', () => {
  it('drives a runtime-backed session through create, send, agent response, and settlement', () => {
    ...
    const fleet: SessionsFleetApi = { enqueueCommand: ({ machineId, projectId, command }) => { const now = new Date('2026-08-30T00:00:00.000Z').toISOString(); database.prepare("INSERT OR IGNORE INTO node_commands(id,machine_id,project_id,operation_key,kind,payload_json,state,created_at,expires_at,updated_at) VALUES (?,?,?,?,?,?,'queued',?,?,?)").run(command.commandId, machineId, projectId, command.operationKey, command.kind, JSON.stringify(command), now, command.expiresAt, now); return { id: command.commandId }; } };
    service.setFleet(fleet);
    ...
    const human = service.queueMessage(one, session.id, { body: 'hello user@example.test api-key=supersecret' });
  ...
  it('routes runtime events by command and native session, sanitizes reserved IDs, and ignores late events', () => {
    ...
    // Fleet reduces terminal runtime events while its command transaction is
  ...
  it('persists provider turn IDs from terminal results and answers runtime approvals', () => {
    ...
    service.setFleet({ enqueueCommand: ({ command: value }) => { queued = value; return { id: value.commandId }; } });
  ...
  it('keeps the provider cancel command deliverable while cancelling a runtime run', () => {
    ...
    service.setFleet({ enqueueCommand: ({ machineId, projectId, command }) => {
  ...
  it('rolls back an approval answer when runtime command enqueue fails', () => {
    ...
    service.setFleet({ enqueueCommand: () => { throw new Error('fleet unavailable'); } });
    expect(() => service.answerApproval(one, session.id, approval.id, { decision: 'deny' })).toThrow('fleet unavailable');
