...
/** Narrow Fleet seam keeps Sessions unit-testable without coupling to Fleet's class. */
export interface SessionsFleetApi {
...
export class SessionsService {
  private fleet: SessionsFleetApi | undefined;
  private transientEventHandler: SessionsTransientEventHandler | undefined;

  constructor(private readonly context: ServerContext, fleet?: SessionsFleetApi) { this.fleet = fleet; }

  setFleet(fleet: SessionsFleetApi): void { this.fleet = fleet; }
  ...
  /** Reconciles durable Fleet command outcomes into the session state machine. */
  maintenance(): void {
  ...
  handleRuntimeEvent(input: { projectId: string; machineId: string; commandId: string; operationKey: string; event: { eventId: string; eventKind: string; payload: Record<string, unknown>; occurredAt: string } }): void {
    ...
    // Fleet reduces terminal command events while its command-state
    ...
    // the ambient transaction when this callback is invoked from Fleet and
  ...
  addParticipant(user: AuthenticatedUser, sessionId: string, participantUserId: string): SessionParticipant {
    ...
    const member = this.context.database.prepare('SELECT 1 FROM team_members WHERE team_id = ? AND user_id = ?').get(session.teamId, participantUserId);
    if (!member) throw new SessionsError(422, 'not_a_team_member', 'Participant must belong to the project team');
    const now = this.now();
    this.context.events.transaction(() => {
      this.context.database.prepare(`
  ...
  queueMessage(user: AuthenticatedUser, sessionId: string, input: QueueMessageInput): SessionMessage {
    ...
    const result = this.context.events.transaction(() => {
      ...
      if (idempotencyKey) {
        ...
        if (existing) {
          const payload = JSON.parse(existing.payload_json) as { messageId?: string };
      ...
      this.append(session.projectId, 'human.message.queued', 'session', sessionId, user, { sessionId, messageId: messageIdValue, sequence: sequenceRow.next_message_sequence, queued: Boolean(activeTurn), includeHumanIdentity: Boolean(input.includeHumanIdentity) }, idempotencyKey);
  ...
  setRunState(user: AuthenticatedUser, runId: string, state: RunState): SessionRun {
    ...
    this.context.events.transaction(() => {
      ...
      if (state === 'settled' && nextState === 'running') {
        this.context.database.prepare("UPDATE agent_activations SET state = 'waiting_on_children', last_activity_at = ? WHERE logical_agent_id IN (SELECT id FROM logical_agents WHERE run_id = ?) AND state = 'running'").run(now, runId);
      }
      if (nextState === 'settled') {
        this.context.database.prepare("UPDATE agent_activations SET state = 'settled', ended_at = ?, last_activity_at = ? WHERE logical_agent_id IN (SELECT id FROM logical_agents WHERE run_id = ?) AND state = 'running'").run(now, now, runId);
      }
  ...
  answerApproval(user: AuthenticatedUser, sessionId: string, approvalId: string, input: AnswerApprovalInput): SessionApproval {
    ...
    if (approval.state === 'pending' && approval.expires_at && Date.parse(approval.expires_at) <= Date.parse(now)) {
      this.context.events.transaction(() => {
        ...
        if (expired) {
          this.context.database.prepare("UPDATE sessions SET state = CASE WHEN state = 'needs_approval' AND NOT EXISTS (SELECT 1 FROM approvals WHERE session_id = ? AND state = 'pending') THEN 'idle' ELSE state END, updated_at = ? WHERE id = ?").run(sessionId, now, sessionId);
  ...
  authorizedEvents(first: AuthenticatedUser | string, second: string, third?: string | number, fourth?: number): EventEnvelope[] {
    ...
    if (typeof first !== 'string') this.authorizeSession(first, sessionId, false);
    return this.context.events.listAfter(projectId, afterSequence)
      .filter((event) => event.aggregateId === sessionId || event.parentAggregateId === sessionId || event.payload.sessionId === sessionId)
  ...
  private expirePendingApprovals(): void {
    ...
    this.context.events.transaction(() => {
      for (const row of rows) {
        ...
        if (!remaining) this.context.database.prepare("UPDATE sessions SET state = CASE WHEN state = 'needs_approval' THEN 'idle' ELSE state END, updated_at = ? WHERE id = ?").run(now, row.session_id);
        const actor = this.systemActor(row.created_by, row.project_id);
  ...
  private commandForOperation<T extends NodeCommand>(machineId: string, operationKey: string, create: () => T): T {
    ...
    if (existing) {
      try { return JSON.parse(existing.payload_json) as T; } catch { /* fall through and let Fleet reject malformed history */ }
    }
    return create();
  }
  ...
  private enqueueCommand(input: { machineId: string; projectId: string; command: NodeCommand }): void {
    if (this.fleet) {
      this.fleet.enqueueCommand(input);
      // Fleet defers transport while the enclosing transaction is open; retry
      // once the commit is visible to the node connection.
      if (this.fleet.deliverPending) queueMicrotask(() => this.fleet?.deliverPending?.(input.machineId));
      return;
    }
    const now = this.now();
  ...
  private authorizeRun(user: AuthenticatedUser, runId: string, write: boolean, allowTerminalRun = false): RunRow & { projectId: string; sessionId: string } {
    ...
    this.assertProjectTeam(row.team_id, user);
    if (write && !this.isParticipant(row.session_id, user.id) && !this.isTeamAdministrator(row.team_id, user.id)) throw new SessionsError(403, 'session_forbidden', 'Session participant access is required');
  ...
  private authorizeSession(user: AuthenticatedUser, sessionId: string, write: boolean): SessionRow & { projectId: string; teamId: string; activeTurnId?: string; runtimeRegistrationId?: string } {
    ...
    this.assertProjectTeam(row.team_id, user);
    const currentMembership = this.context.database.prepare('SELECT u.id, tm.role FROM users u JOIN team_members tm ON tm.user_id = u.id AND tm.team_id = ? WHERE u.id = ? AND u.disabled_at IS NULL').get(row.team_id, user.id) as { id: string; role: AuthenticatedUser['role'] } | undefined;
    if (!currentMembership) throw new SessionsError(403, 'project_forbidden', 'Project access is not authorized');
    const participant = this.isParticipant(sessionId, user.id);
    if (!participant && currentMembership.role !== 'administrator') throw new SessionsError(403, 'session_forbidden', 'Session participant access is required');
  ...
  private assertProjectMember(projectId: string, user: AuthenticatedUser): void {
    ...
    this.assertProjectTeam(project.team_id, user);
  }
  ...
  private assertProjectTeam(teamId: string, user: AuthenticatedUser): void {
    if (teamId !== user.teamId) throw new SessionsError(403, 'project_forbidden', 'Project access is not authorized');
  }
...
function redactRuntimeIdentifiers(value: Record<string, unknown>, depth = 0): Record<string, unknown> {
  ...
  for (const [key, item] of Object.entries(value).slice(0, 128)) {
    ...
    if (Array.isArray(item)) output[key] = item.slice(0, 128).map((entry) => entry && typeof entry === 'object' && !Array.isArray(entry) ? redactRuntimeIdentifiers(entry as Record<string, unknown>, depth + 1) : entry);
    else if (item && typeof item === 'object') output[key] = redactRuntimeIdentifiers(item as Record<string, unknown>, depth + 1);
