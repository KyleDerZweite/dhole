...
import { SessionsService } from '../apps/server/dist/modules/sessions/service.js';
...
try {
  ...
  const enrollment = fleet.issueEnrollmentToken({ teamId: 'team', label: 'session-node', createdBy: 'user' });
  const device = fleet.consumeEnrollmentToken(enrollment.token);
  ...
  const service = new SessionsService(context, fleet);
  ...
  }, () => `Interactive session did not settle: ${JSON.stringify({ summary: service.getSummary(session.id), commands: fleet.listCommands(device.machineId) })}`);
