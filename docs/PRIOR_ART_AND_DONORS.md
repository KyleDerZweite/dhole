...
| [Hermes Agent](https://github.com/NousResearch/hermes-agent) | Persistent sessions, skills and memory loops, FTS-backed recall, remote messaging, and bounded subagent delegation. | The original design explored these concepts. ADR 0003 keeps sessions in Core and retires server Memory, Skills, and orchestration from the active product. |
...
| [Agent Skills](https://github.com/agentskills/agentskills) | Portable `SKILL.md` packaging, references, optional scripts, and a simple discovery/activation model. | Dhole maintains static client instructions in `clients/skills` with explicit installation and removal. There is no active server skill registry or benchmark promotion flow. |
